...
```{r, fig.dim=c(4,3)}
barplot(results/trials)
```

## Estimating Distributions via Simulation

::: {.example #flip-with-fib-R name="Flipping Coins with Fibonacci"}
...
We note that the maximum number of flips it took to get consecutive Heads in these trials was `max(results)` = `r max(results)`! So we don't list the full table of results here, just the first 10:

```{r}
table(results)[1:10]
```

The **relative frequencies** of these results provide our estimate for the probability density function:
