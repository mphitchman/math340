...
2.  We want code to check whether a random sequence of 5 rolls is *good*.
...
Recal, the `sum()` of a logical vector counts the number of TRUE values. We need the sum to be 5 to have a *good* sequence of 5 rolls.

```{r}
sum(c(rolls[1:4]<8,rolls[5]>=8))
```
...
Based on our simulation, we estimate the probability of a *good* sequence of 5 rolls by the proportion of time the `results` vector took a value of 5.
...
Conclusion: It appears we should expect the difference in the two dice to be 1 about `r round(100*sum(results==1)/trials,1)`% of the time.
...
One approach:
  ...
  - Build a vector for recording outcome of five flips: `flips = sample(coin,size=5,replace=TRUE)`
  - Find how many of these five flips are heads with `sum(flips=="H")`
  - Repeat!
...
Conclusion: It appears we should expect to flip exactly 1 heads in 5 tries about `r round(100*sum(heads==1)/trials,1)`% of the time.
...
1.  Build a random plate. We want three random numbers (repeats ok), followed by three random letters (repeats ok). This means we sample from 0-9 with replacement 3 times, then sample from A-Z with replacement 3 times. R comes with some built-in data sets and vectors, and one of them is `LETTERS` (the vector `c('A','B',...,'Z')`).
...
2.  We can record the number of "8"s in the plate
...
As the display above suggests, we can extract a value from matrix `m` by indicating its [row,column]:
...
> a) In our family of 6, what is the probability we will have to redraw.

Our approach: Create a family vector (called `family`), and a permutation of `family` (called `draw`) and see whether these two vectors agree in any spot - if so, we need to redraw.
...
sum(results)/trials
```
About a `r round(sum(results)/trials*100,1)` percent chance we'll need to redraw.
...
Cripes! Can you believe it took `r max(results)` draws on one of the trials? Anyway, our estimate for the expected number of draws needed to get the gift-making off the mark is:

```{r}
sum(results)/trials
```
Well THAT number looks somewhat familiar ... .
...
family_size = c(2:10,seq(15,50,5))
prob_redraw = c()
expected_draws = c()
