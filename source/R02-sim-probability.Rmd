...
By constructing a 6x6 grid recording the difference for each of the 36 possible outcomes, we would find the actual probability equal to 10/36 $\approx$ `r round(10/36,3)`.
...
<!-- exact answer: 10/36 (from 6x6 grid) -->
...
for (i in 1:trials){
  ...
  #are both "I"s in the same subgroup? We record 1 if "yes", and 0 if "no"
...
1.  Build a random plate. We want three random numbers (repeats ok), followed by three random letters (repeats ok). This means we sample from 0-9 with replacement 3 times, then sample from A-Z with replacement 3 times. R comes with some built-in data sets and vectors, and one of them is `LETTERS`!
