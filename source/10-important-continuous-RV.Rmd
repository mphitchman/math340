...
A random variable $X$ has an **exponential probability distribution with parameter $\beta$**\index{exponential distribution}, denoted $\text{Exp}(\beta)$\index{Exp$(\beta)$}, if it has probability density function 
$$
f(x)=
...
If $X$ is $\text{Exp}(\beta))$, then $$E(X) = \beta, ~~~ \text{ and } ~~~ V(X) = \beta^2.$$
:::
...
A random variable $X$ has a **gamma probability distribution with parameters $\alpha>0$ and $\beta>0$**\index{gamma distribution} if and only if it has probability density function 
$$
f(x)=
...
:::{.theorem #gamma-EandV}
If $X$ is $\text{gamma}(\alpha,\beta))$, then $$E(X) = \alpha\beta, ~~~ \text{ and } ~~~ V(X) = \alpha\beta^2.$$
:::
...
:::{.theorem #beta-EandV}
If $X$ is $\text{beta}(\alpha,\beta))$, then $$E(X) = \frac{\alpha}{\alpha+\beta}, ~~~ \text{ and } ~~~ V(X) = \frac{\alpha\beta}{(\alpha+\beta)^2(\alpha+\beta+1)}.$$
:::
